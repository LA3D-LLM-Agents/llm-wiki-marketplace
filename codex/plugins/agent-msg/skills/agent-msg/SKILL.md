...
You operate the cross-harness messaging tool: two scripts under `$SKILL_DIRECTORY/scripts/`. It lists sessions from both harnesses and sends a message to one, routed by the target's platform. Same machine only; a message just appears in the recipient (a queued turn for Codex, a pop-up for a live Claude session).
...
```bash
bash "$SKILL_DIRECTORY/scripts/agents-list.sh"
```

Prints one row per agent, tagged by platform (`claude` / `codex`), with its id, name, cwd, and status. Recent Codex sessions come from `~/.codex/sessions`; live Claude sessions from `claude agents --json`. Add `--tsv` for machine-readable output.

## Send a message

```bash
bash "$SKILL_DIRECTORY/scripts/agents-send.sh" <target> "<message>"
```
