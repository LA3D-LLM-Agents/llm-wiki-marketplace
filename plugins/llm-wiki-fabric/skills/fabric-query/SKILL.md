...
2. Call `fabric_identify` for the selected resource. Inspect `ontology_profile` (fabric 0.2.0), `capability_details`, `services`, `semantic_entrypoints`, and `access_requirements`. Capability IDs/names/descriptions/tags describe advertisements, not permission or a guarantee that this connector supports every operation. Use `connection` for the selected resource route; a card or OpenAPI URL is not an MCP invocation endpoint. Follow semantic entrypoint execution location: PAD ontology tools run at PAD, while database dictionary/live-schema tools run in the local direct connector. PAD’s domain ontology is independent of fabric’s structural ontology. Check descriptor freshness: disclose `stale` or `offline-cache` metadata, and do not interpret published DID metadata as verified identity. Retain its `resource_id` and `revision`; pass both to direct connector tools. If a tool returns `stale_catalog`, rediscover after both servers have restarted with the same catalog.
...
Snapshot schema 2 requires plugin/runtime 0.7.0 or later. After a catalog revision change, restart the direct connector and rediscover before querying. Do not replace a revision returned by discovery with an older remembered revision.
